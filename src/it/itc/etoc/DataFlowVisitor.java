...
public class DataFlowVisitor extends ParseTreeVisitor {
  ...
  /**
   * The backward link to the DataFlowInstrumentor class is used 
   * to check if a Variable is a field.
   */
  ...
  public  void 	visit(ForStatement p) throws ParseTreeException {
    ...
    if (incr != null) incr.accept(this);
    ExpressionList init = p.getInit();
    if (init != null) init.accept(this);
    StatementList stmts = p.getStatements();
    if (stmts != null) stmts.accept(this);
  }
  ...
  public  void 	visit(UnaryExpression p) throws ParseTreeException {
    Expression expr = p.getExpression();
    if (p.getOperator() == p.POST_DECREMENT ||
	p.getOperator() == p.POST_INCREMENT ||
	p.getOperator() == p.PRE_DECREMENT ||
	p.getOperator() == p.PRE_DECREMENT)
      isDef = true;
    isUse = true;
    expr.accept(this);    
  ...
  public  void 	visit(Variable p) throws ParseTreeException {
    if (instrumentor.isField(p.toString())) {
      if (isDef)
	instrumentor.addDef(
		((Integer)lastBranchCounter.peek()).intValue(),
		p.toString());
      if (isUse)
	instrumentor.addUse(
		((Integer)lastBranchCounter.peek()).intValue(),
